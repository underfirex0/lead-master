...
import {
  ...
  Crown, Phone, Mail, MapPin, Target,
  Shield, Star, PlayCircle, Globe,
  BarChart2, CalendarClock, Filter, Unlock
} from 'lucide-react'
import { cn } from '@/lib/utils'

// Register ScrollTrigger once (no useGSAP plugin needed)
        ...
        {mobileOpen && (
          <div className="md:hidden mt-2 bg-white border border-[rgba(0,0,0,0.07)] rounded-2xl shadow-floating p-3 animate-scale-in">
            ...
            <div className="border-t border-[rgba(0,0,0,0.06)] mt-2 pt-2 space-y-2">
              <Link href="/login" className="block text-[14px] text-center font-medium text-ink-3 py-3 hover:bg-surface-1 rounded-xl">
                Se connecter
              </Link>
              <Link href="/register" className="btn-brand w-full justify-center text-sm">
                Commencer gratuitement
              </Link>
            </div>
          </div>
    ...
    <div className="relative w-full max-w-[580px]">
      ...
      <div className="rounded-[18px] overflow-hidden bg-white shadow-[0_24px_80px_rgba(0,0,0,0.12),0_0_0_1px_rgba(0,0,0,0.05)]"
          ...
          {rows.map((row, i) => (
            <div key={row.name} className={cn('grid grid-cols-[2fr_0.8fr_1fr_1fr] px-4 py-2.5 border-b border-[rgba(0,0,0,0.04)] hover:bg-[rgba(79,70,229,0.025)] transition-colors', i === rows.length-1 && 'border-b-0')}>
              ...
              <div className="text-[10px] text-ink-3 flex items-center gap-1">
                <MapPin className="w-2.5 h-2.5 text-ink-5 shrink-0" /><span className="truncate">{row.city}</span>
              </div>
        ...
        <div className="bg-surface-1 border-t border-[rgba(0,0,0,0.05)] px-4 py-2 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5">
            <span className="inline-flex items-center gap-1 text-[9px] font-semibold text-brand-600 bg-brand-50 px-2 py-1 rounded-pill border border-brand-100">
              <Users2 className="w-2.5 h-2.5" /> Ajouter au CRM
            </span>
            <span className="inline-flex items-center gap-1 text-[9px] font-semibold text-ink-3 bg-white px-2 py-1 rounded-pill border border-[rgba(0,0,0,0.08)]">
...
function Hero() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl.from(headRef.current,   { opacity: 0, y: 44, duration: 0.95 }, 0.1)
      ...
      counters?.forEach(el => {
        ...
        const obj    = { v: 0 }
        gsap.to(obj, {
          v: target, duration: 1.8, ease: 'power2.out', delay: 0.8,
          onUpdate: () => {
            el.textContent = Math.round(obj.v).toLocaleString('fr-MA') + suffix
          },
        })
      })
    }, containerRef)
        ...
        <div ref={statsRef} className="grid grid-cols-2 sm:grid-cols-4 gap-6 py-8 border-t border-[rgba(0,0,0,0.06)]">
          {[
            ...
            <div key={label} className="text-center sm:text-left">
              <span className="block text-[30px] sm:text-[34px] font-extrabold text-ink-1 tabular-nums"
...
function DataStrip() {
  const items = [
    ...
    '95% avec téléphone direct','82 contacts direction DAF · DRH',
...
function Features() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.from(el.querySelectorAll('.feat-card'), {
...
function FieldPricing() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.from(el.querySelectorAll('.field-row'), {
              ...
              <div key={label} className="field-row bg-[#FAFAF9] border border-[rgba(0,0,0,0.06)] rounded-xl p-4 hover:border-[rgba(0,0,0,0.1)] hover:shadow-xs transition-all">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[13px] font-semibold text-ink-2">Tier {idx + 1}</span>
                  </div>
...
function Pricing() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.from(el.querySelectorAll('.plan-card'), {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-start">
          {plans.map(plan => (
            <div key={plan.id} className={cn(
              ...
              plan.hot ? 'bg-brand-600 border-brand-500 shadow-[0_8px_40px_rgba(79,70,229,0.25)] lg:scale-[1.04] lg:-translate-y-1' : 'bg-white hover:shadow-card-md hover:-translate-y-0.5',
              plan.color
            )}>
              {plan.badge && (
                <div className={cn('text-[10px] font-bold uppercase tracking-widest mb-3', plan.hot ? 'text-brand-200' : plan.id === 'business' ? 'text-gold-600' : 'text-brand-600')}>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              <Link href="/meetmaster" className="btn-gold btn-lg group">
                Explorer les Masters <ArrowRight className="w-4 h-4 group-hover:translate-x-0.5 transition-transform" />
              </Link>
    ...
    <section ref={sRef} className="py-24 px-5 bg-white">
      <div className="max-w-[1200px] mx-auto">
        <div className="text-center mb-14">
          <h2 className="font-extrabold text-ink-1" style={{ fontSize: 'clamp(26px,3.5vw,40px)', letterSpacing: '-1px', lineHeight: 1.2 }}>
  ...
  return (
    <section id="faq" className="py-24 px-5 bg-[#F7F7F5]">
      <div className="max-w-[760px] mx-auto">
        <div className="text-center mb-14">
          <h2 className="font-extrabold text-ink-1" style={{ fontSize: 'clamp(26px,3.5vw,40px)', letterSpacing: '-1px', lineHeight: 1.2 }}>Questions fréquentes</h2>
        </div>
        <div className="space-y-2">
          {faqs.map(({ q, a }, i) => (
            <div key={i} className={cn('bg-white rounded-xl border overflow-hidden transition-all duration-200',
              open === i ? 'border-brand-200 shadow-[0_2px_12px_rgba(79,70,229,0.07)]' : 'border-[rgba(0,0,0,0.07)] hover:border-[rgba(0,0,0,0.12)]')}>
              <button onClick={() => setOpen(open === i ? null : i)}
                className="w-full flex items-center justify-between px-6 py-5 text-left gap-4" aria-expanded={open === i}>
  ...
  return (
    <section ref={sRef} className="py-28 px-5 bg-white">
      <div className="max-w-[780px] mx-auto text-center">
        ...
        <h2 className="cta-item font-extrabold text-ink-1 mb-5" style={{ fontSize: 'clamp(34px,5vw,60px)', letterSpacing: '-2.5px', lineHeight: 1.05 }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-10 mb-14">
          <div className="col-span-2 md:col-span-1">
            ...
            <div className="flex items-center gap-1.5 text-[11px] text-[rgba(255,255,255,0.3)]">
              <Globe className="w-3 h-3" /><span>Casablanca · Maroc</span>
            </div>
          </div>
